...
const getWrapper = ({ url, method = "GET", thisInstance, config = {} }) => {
	const fullUrl = buildUrl(url, thisInstance);
	const headers = buildHeaders(config, thisInstance);

	config.headers = headers;

	const hasWindow = typeof window !== "undefined";
	const fncRequest = hasWindow
		? require("../utils/requestWeb")
		: require("../utils/requestNode");

	const promise = fncRequest({
		method,
		fullUrl: fullUrl,
		config: config,
	...
	return raceTimeout(promise, config, thisInstance);
};
...
class Dame {
	...
	get(url, config = {}) {
		return getWrapper({ url, config, method: "GET", thisInstance: this });
	}
	...
	delete(url, config) {
		return getWrapper({ url, config, method: "DELETE", thisInstance: this });
	}
